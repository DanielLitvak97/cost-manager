...
// Get all stored costs from the database.
function getAllStoredCosts() {
    const database = db.openCostsDB("costsdb", 1);

    if (typeof database.getAllCosts === "function") {
...
// Convert a cost into the requested currency.
function convertCost(cost, targetCurrency, exchangeRates) {
    const costInUsd = cost.sum / exchangeRates[cost.currency];

    return costInUsd * exchangeRates[targetCurrency];
}

// Create totals according to categories.
function createCategoryTotals(costs, targetCurrency, exchangeRates) {
...
// Create totals according to months.
function createMonthlyTotals(costs, year, targetCurrency, exchangeRates) {
...
function createBarChart(monthlyTotals, currency, year) {
    const labels = [
        "January",
        "February",
        "March",
