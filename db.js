(function () {
    ...
    function convertCost(
        ...
        if (
            !supportedCurrencies .includes(originalCurrency) ||
     ...
     *
     * The required API is db.getReport().
    ...
    /*
     * The standalone Vanilla library loads the default exchange rates.
     *
     * Production pages set data-skip-default-rates="true" on the
     * db.js script because currency.js manages exchange-rate loading there.
     */
    const currentScript = document.currentScript;

    const shouldLoadDefaultExchangeRates =
        !(
            currentScript &&
            currentScript.dataset.skipDefaultRates === 'true'
        );

    if (shouldLoadDefaultExchangeRates) {
